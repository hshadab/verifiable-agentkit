...
# Script to update index.html with improved zkEngine UI
# Run this from your ~/agentkit/ directory

echo "🔄 Updating index.html with improved zkEngine UI..."

# Check if we're in the right directory
if [[ ! -f "Cargo.toml" ]]; then
    echo "❌ Error: Please run this script from the ~/agentkit/ directory"
    echo "   Current directory: $(pwd)"
    echo "   Expected to find: Cargo.toml"
    exit 1
fi

# Create backup of existing index.html
if [[ -f "static/index.html" ]]; then
    echo "📦 Creating backup of existing index.html..."
    cp static/index.html static/index.html.backup.$(date +%Y%m%d_%H%M%S)
    echo "✅ Backup created: static/index.html.backup.$(date +%Y%m%d_%H%M%S)"
fi

# Ensure static directory exists
mkdir -p static

# Create the updated index.html file
...
<body>
    ...
    <script>
        ...
        function toggleProgram(proofId, wasmFile, functionName) {
            const codeViewer = document.getElementById(`code-viewer-${proofId}`);
            const codeContent = document.getElementById(`code-content-${proofId}`);
            
            if (codeViewer.classList.contains('show')) {
                codeViewer.classList.remove('show');
                return;
            }
            ...
            codeContent.textContent = wasmCode;
        }
...
echo "📁 Location: $(pwd)/static/index.html"
echo ""
echo "🚀 Changes made:"
echo "   • Fixed View Program functionality with real WASM code"
echo "   • Replaced modal with inline expandable code viewer"
echo "   • Added smooth slide-down animations"
echo "   • Code now shows directly below each proof card"
echo ""
echo "🔄 Next steps:"
echo "   1. Restart your Rust backend: cargo run"
echo "   2. Open http://localhost:8001 in your browser"
echo "   3. Test the 'View Program' button on proof cards"
echo ""
echo "✨ Done! Your zkEngine UI is now updated with improved code viewing."
